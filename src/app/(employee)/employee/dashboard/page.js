...
export default function EmployeeDashboardPage() {
  ...
  const [todayRec, setTodayRec] = useState(undefined); // undefined = loading, null = none
  ...
  // Fix hydration: Add mounted state for client-only values
  ...
  // Fix hydration: Set client-only values after mount
  useEffect(() => {
    setMounted(true);
  ...
  useEffect(() => {
    ...
    const tick = () =>
      setElapsed(
        (Date.now() - new Date(todayRec.checkIn).getTime()) / 3_600_000,
      );
    tick();
  ...
  // Socket: refresh when admin reviews our leave
  useEffect(() => {
  ...
  const handleCheckIn = async () => {
    ...
    if (res.ok) {
      showToast(
        data.isLate
          ? `Checked in — marked Late ⚠️`
          : "Checked in successfully ✅",
        data.isLate ? "warning" : "success",
      );
      fetchAll();
    } else {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 22 }}>
      ...
      <Card
        style={{
          background:
            "linear-gradient(135deg,rgba(79,142,247,.07),rgba(124,92,252,.07))",
      ...
      >
        ...
        >
          <div>
            ...
            {todayRec?.isLate && (
              <div
                style={{ fontSize: 12, color: "var(--warning)", marginTop: 5 }}
              >
          ...
          {[
            {
              icon: "🕗",
              label: "Check In",
              value: formatTime(todayRec?.checkIn),
            },
            {
              icon: "🕔",
              label: "Check Out",
              value: formatTime(todayRec?.checkOut),
            },
            {
              icon: "⏱️",
              label: "Hours Today",
              value:
                isCheckedIn && elapsed != null
                  ? formatHours(elapsed)
                  : formatHours(todayRec?.hoursWorked),
      ...
      >
        ...
        <StatCard
          icon="⚠️"
          label="Late"
          value={monthAtt.filter((a) => a.isLate).length}
          sub="This month"
          color="var(--warning)"
        />
        <StatCard
          ...
          label="Casual Leave"
          value={avail("CL")}
          sub={`of ${balance?.clTotal || 12} days`}
          color="var(--accent)"
        />
        <StatCard
          icon="🏥"
          label="Sick Leave"
          value={avail("SL")}
          sub={`of ${balance?.slTotal || 10} days`}
          color="var(--accent2)"
        />
      </div>
        ...
        <Table
          cols={[
            ...
            },
            {
              key: "status",
              label: "Status",
              render: (r) => <Badge status={resolveAttStatus(r)} />,
            },
          ]}
          rows={[...monthAtt]
          ...
          cols={[
            {
              ...
              label: "Type",
              render: (r) =>
                `${LEAVE_CONFIG[r.type]?.emoji || ""} ${LEAVE_CONFIG[r.type]?.label || r.type}`,
            },
            {
              key: "from",
              label: "From",
            ...
            { key: "days", label: "Days" },
            {
              key: "status",
              label: "Status",
              render: (r) => <Badge status={r.status?.toLowerCase()} />,
            },
          ]}
          rows={myLeaves}
