...
const leaveService = {
  ...
  async apply(userId, { type, startDate, endDate, reason }) {
    ...
    const avail =
      balance[cols.total] - balance[cols.used] - balance[cols.pending];
    if (days > avail) {
      throw new ApiError(
    ...
    if (overlap)
      throw new ApiError("Overlapping leave request exists", 409, "OVERLAP");
  ...
  async review(leaveId, adminId, { action, reviewNote }) {
    ...
    if (leave.status !== "PENDING")
      throw new ApiError("Leave already reviewed", 409);
