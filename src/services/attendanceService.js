...
const attendanceService = {
  ...
  async autoCheckoutOverdue() {
    const cfg = await this.getConfig();
    const cutoff = new Date(Date.now() - cfg.autoCheckoutHours * 3_600_000);
    const today = this.todayDate();

    const overdue = await db.attendance.findMany({
      where: { checkOut: null, checkIn: { lte: cutoff }, date: today },
    });

    let count = 0;
    for (const rec of overdue) {
      const checkOut = new Date(
        rec.checkIn.getTime() + cfg.autoCheckoutHours * 3_600_000,
      );
